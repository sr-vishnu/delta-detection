...
WHERE customer_profile.source_id = n.source_id
    AND customer_profile.source_type = n.source_type
    AND customer_profile.status = 'CURRENT';
...
new_rows_ranked AS (
    SELECT
        *,
        ROW_NUMBER() OVER (
            PARTITION BY customer_profile.source_id, customer_profile.source_type
...
calculated_actions AS (
    SELECT
        ...
        -- if a row is the first in its paritition then if this pariticular entity was not present in the previous state of the currrent set which we snapshoted before (current_state)
        -- then we we know its new and we set its action type to INSERT , but if its present in the previous current set (current_state) , then it can either be one of UPDATE or NOOP (no operation)
        -- depending on if the payload has changed since its previous state
        ...
        CASE
            WHEN n.row_num_asc = 1 THEN
                CASE
                    WHEN e.customer_profile.source_id IS NULL THEN 'INSERT'
    ...
    LEFT JOIN current_state e
        ON n.customer_profile.source_id = e.customer_profile.source_id
       AND n.customer_profile.source_type = e.customer_profile.source_type
),
